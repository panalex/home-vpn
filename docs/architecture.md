...
**fronting резолвит и коннектит НАПРЯМУЮ с RU edge.** mtg фронтит через `native`
диалер, минуя SOCKS-цепочку (`mtglib/proxy.go`), поэтому и резолв хоста через
`[network].dns`, и сам коннект к нему идут с RU IP edge напрямую. Из-за этого
`mtg_dns` обязан быть **RU-достижим**: дефолтный Cloudflare DoH `1.1.1.1` из РФ
режется ТСПУ (`dial tcp 1.1.1.1:443: connect: no route to host`) → fronting падал
с `cannot dial to the fronting domain`. Поэтому `mtg_dns = udp://77.88.8.8` (Yandex,
тот же резолвер, что `ru_dns_dot_ip`): он резолвит `uniqr.pay.yandex.net`, а прямой
коннект RU edge → Yandex:443 проходит (RU→RU), и зонд получает валидный yandex-серт.
Обычные клиенты `mtg_dns` НЕ используют — их трафик уходит в SOCKS-цепочку и
резолвится на гейтвее, поэтому смена резолвера egress не трогает.

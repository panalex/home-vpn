...
# Edge inbounds for the home→edge leg (hy2-in/vless-reality-in/shadowtls-in) + firewall
# (the ru_singbox tag also runs reality_keys → generates secrets/reality.yml; firewall opens tcp/443)
...
**Edge VPS** (`ru_vps`): public ingress. Runs nginx (ACME for the edge LE cert) and `sing-box` (HTTP proxy :2080, SOCKS :2081 with auth, local loopback :1080/:1081). All proxy traffic chains to the gateway over the edge→DE uplink (shadowtls v3 by default, `de_uplink_transport`). :443 on edge now hosts `vless-reality-in` (VLESS+Reality+gRPC, masquerading as `reality_donor_sni`) — a TCP whitelist/insurance leg for the home→edge hop alongside hy2 (the former Telegram MTProto/`mtg` stack on :443 was removed).
...
**Router** (`routers`): GL-MT6000 with OpenWrt + `sing-box` TUN (`singtun0`), deployed by the **`owrt_singbox`** + **`owrt_nfqws`** roles (the previous `flint_*` roles were dropped — over-debugged and unreliable; rebuilt clean as `owrt_*`). Split routing (by sniffed SNI + geoip): RU IPs/domains go direct, everything else (incl. RKN-blocked sites by default) tunnels to gateway via the `proxy` outbound. **The home→edge leg uses hysteria2** — raw SS2022 `router-in` :8388 was getting cut by the ISP TSPU (TCP handshake passes, first ciphertext block dropped → blackhole), so edge now exposes three always-on inbounds: `hy2-in` (hysteria2 over QUIC/UDP + obfs salamander, **primary**), `vless-reality-in` (VLESS+Reality+gRPC on tcp/443, TCP whitelist/insurance) and `shadowtls-in` (shadowtls v3 → internal SS2022, TCP fallback). All route into the edge→DE uplink on edge (active outbound selected by `de_uplink_transport`). **The router transport is a manual switch** (`router_primary_transport`, mirroring `de_uplink_transport` on edge): `owrt_singbox` renders the matching outbound as tag `proxy` — `hysteria2` (primary), a `reality` VLESS+Reality+gRPC outbound (TCP/:443, whitelist/UDP-cut insurance), or a `shadowtls` v3 → internal SS2022 pair (`proxy-stls` + `proxy`, TCP fallback for when TSPU cuts QUIC/UDP) — and asserts the var is one of those three. **Why gRPC specifically:** it gives HTTP/2-mux so the fan-out of LAN flows from the router TUN collapses into 1–2 connections to a single SNI (kills the "many TCP to one SNI/IP" behavioral trigger); NOT XHTTP (sing-box core doesn't support it — 0 bytes received), NOT bare TCP+Reality (TSPU signature-detects it since May 2026). Reality forwards the real TLS1.3 handshake to `reality_donor_sni` (default `ya.ru`, auto-selected from the edge by TLS1.3+h2+RTT), so an active probe of edge:443 sees the genuine donor site. The Reality keypair/short_id/uuid are generated once by the `reality_keys` role into `secrets/reality.yml` (gitignored, idempotent). Edge listens on both inbounds always, so switching = change the var + redeploy the router only (edge untouched). The router shadowtls outbound runs **without multiplex** to stay symmetric with the edge `shadowtls-ss-in` inbound (which has none) — adding mux on one end only blackholes the return channel. Brutal is intentionally OFF (no `up_mbps`/`down_mbps`) — fixed-rate CC produces an anomalous flat traffic pattern that aids behavioral detection. `dpi_bypass_domains` (default empty) is an opt-in third category: listed domains go direct WAN + nfqws DPI-desync instead of the tunnel. Split-DNS: `*.ru` + `geosite-ru` + `dpi_bypass_domains` resolve via Yandex **DoT** (`dns-local`, anti-spoofing, direct WAN); **everything else resolves via DoH over the tunnel** (`dns-remote` = 1.1.1.1, `detour: proxy`) — real IPs resolved at the gateway, no FakeIP. A cron watchdog (`/usr/bin/singbox-watchdog`) restarts `sing-box` if the process dies.
...
- `router_primary_transport` — active home→edge transport on the router: `hysteria2` (primary, UDP/QUIC) | `reality` (VLESS+Reality+gRPC on tcp/443, whitelist/UDP-cut insurance) | `shadowtls` (fallback, TCP). `owrt_singbox` renders the matching `proxy` outbound and asserts the var is one of the three. Edge always listens on all three inbounds, so switching = change the var + redeploy the router only. Keep `hysteria2` unless the ISP cuts QUIC/UDP or moves to a whitelist regime (then `reality`)
- `reality_donor_sni` / `reality_grpc_service_name` — VLESS+Reality donor SNI (default `ya.ru`, auto-selected from the edge by TLS1.3+ALPN-h2+RTT; never a foreign SNI or gov domain) and the gRPC `service_name` (arbitrary, invisible to DPI inside the Reality tunnel, must match both ends). Wired on edge (`vless-reality-in` :443) and the router outbound when `router_primary_transport: reality`
- `reality_keypair_regenerate` — set `true` to rotate the Reality keypair on next run (then redeploy edge + router + re-issue client configs). Default `false`. The `reality_keys` role generates `reality_private_key`/`reality_public_key`/`reality_short_id`/`reality_uuid` once into `secrets/reality.yml` (gitignored) — do not set these by hand; `public_key`/`short_id` for clients live there
...
ss -lntp | grep ':443'                     # vless-reality-in (TCP) — whitelist/insurance (edge always on; router renders it when router_primary_transport: reality)
...
# active-probe: edge:443 must behave like the real donor (Reality forwards the handshake)
echo | openssl s_client -connect <edge_ip>:443 -servername <reality_donor_sni> 2>/dev/null | openssl x509 -noout -subject -issuer
```
